...
<https://docs.google.com/presentation/d/1ZpfVf8J23BpR8bHAjdAWzFS__gvRCf0dOBLwDcDutCg/edit?usp=sharing>
...
With appropriate scale/shift values (e.g. `scale=1.05, shift=-0.025`, YOLOv4 default settings), the function can reach 1 at a finite input (~3.7), fundamentally eliminating the need for infinitely large weights.
...
Directly replacing standard Sigmoid with Scaled Sigmoid in a CNN yields measurably better accuracy and smaller, faster-decreasing weights. Reason: Using scaled sigmoid functions can help reduce gradient vanishing, improving model performance.
...
In many practical scenarios, these issues remain hidden or mitigated:
- **Most cases remain unaffected**: Many standard tasks do not require outputs at the extreme boundaries.
- **Vanishing gradients**: The mechanism of gradient vanishing can act as a natural regulator, preventing weight explosion.
- **Floating-Point Saturation**: Standard Sigmoid saturates to exactly $1.0$ for inputs $\ge 40$ due to floating-point precision limits, masking potential divergence.
...
├── Main.py               # Test cases: simple threshold → XOR → binary encoding (Test Case 4 Main Entry)
├── DemoRecovery.py       # Recovery from weight explosion demo (Faster convergence)
├── LeNet5.py             # LeNet-5 CNN experiments
├── VggCifar10.py         # VGG on CIFAR-10 (CNN Results Entry Point)
├── LstmSeqMnist.py       # LSTM on Sequential MNIST
├── LstmCharGen.py        # LSTM character generation (LSTM Results Entry Point)
...
Each Git tag in this repository showcases the visual results of a specific experiment.
...
# Run test case 4 experiments
python Main.py

# Run CNN experiments
python VggCifar10.py

# Run LSTM character generation experiments
python LstmCharGen.py
